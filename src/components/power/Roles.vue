<template>
  <div>
      ...
      <el-table :data="rolelist" border stripe>
            ...
            <el-row :class="['bdbottom', i1 === 0 ? 'bdtop' : '', 'vcenter']" :key="item1.id"
              ...
              <el-col :span="19">
                ...
                <el-row :class="[i2 === 0 ? '' : 'bdtop', 'vcenter']" :key="item2.id"
                  ...
                  <el-col :span="18">
                    <el-tag :key="item3.id" @close="removeRightById(scope.row, item3.id)" closable type="warning"
                            v-for="item3 in item2.children">{{item3.authName}}
                    </el-tag>
                  </el-col>
                </el-row>
              </el-col>
        ...
        <el-table-column label="操作" width="300px">
          <template slot-scope="scope">
            ...
            <el-button @click="showSetRightDialog(scope.row)" icon="el-icon-setting" size="mini" type="warning">分配权限
            </el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <!-- 分配权限的对话框 -->
    <el-dialog :visible.sync="setRightDialogVisible" @close="setRightDialogClosed" title="分配权限" width="50%">
      <!-- 树形控件 -->
      <el-tree :data="rightslist" :default-checked-keys="defKeys" :props="treeProps" default-expand-all node-key="id"
               ref="treeRef" show-checkbox></el-tree>
